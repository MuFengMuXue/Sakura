...
async def add_preference(
    ...
    try:
        ...
        pref = await preference_memory.add_preference(
            ...
            reason=request.reason,  # 原 v2 支持 reason 字段
        )
        return {
            "status": "success",
...
async def extract_preferences(
    ...
    try:
        # 修正导入路径：从项目根目录的 utils 导入
        from ...utils.entity_extractor import PreferenceExtractor

        extractor = PreferenceExtractor(llm_config=llm_cfg.model_dump())
