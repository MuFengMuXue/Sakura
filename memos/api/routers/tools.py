...
async def record_tool_usage(
    request: RecordToolUsageRequest,
    registry: ServiceRegistry = Depends(get_registry),
):
    """记录工具使用"""
    tool_memory = registry.tool_memory
    ...
    try:
        ...
        # 请求中的 user_id 字段目前未使用，保持与 v2 一致。
...
async def suggest_tool_parameters(
    tool_name: str,
    registry: ServiceRegistry = Depends(get_registry),
):
    """根据历史使用建议工具参数"""
