    ...
    try:
        ...
        if request.store_to_graph and graph and graph.is_available():
            ...
            for entity in entities:
                ...
                # 检查是否已存在（注意 await）
                existing = await graph.find_entity_by_name(entity.name, user_id)
                if existing:
                    ent_id = existing['id']
                    if request.link_to_memory_id and hasattr(graph, 'link_entity_to_memory'):
                        await graph.link_entity_to_memory(ent_id, request.link_to_memory_id)
                else:
                    props = {
...
async def extract_entities_from_all_memories(
    dry_run: bool = False,
    limit: int = 10000,
    user_id: Optional[str] = Query(None, description="用户ID，不指定则使用默认"),
    registry: ServiceRegistry = Depends(get_registry),
):
    ...
    try:
        ...
        for idx, mem in enumerate(all_memories):
            ...
            logger.info(f"[{idx+1}/{total_memories}] 处理: {content[:50]}...")
            ...
            try:
                ...
                for entity in entities:
                    try:
                        ...
                        # 检查是否已存在（await）
                        existing = await graph.find_entity_by_name(entity_name, user_id)
                        if existing:
                            entities_skipped += 1
                        ...
                        # 使用 add_entity
                        success = await graph.add_entity(
                ...
                if relations:
                    for rel in relations:
                        try:
                            ...
                            # 查找源和目标实体（await）
                            source_entity = await graph.find_entity_by_name(source_name, user_id)
                            target_entity = await graph.find_entity_by_name(target_name, user_id)
...
async def get_entity_stats(
    ...
    if graph and graph.is_available():
        user_id = user_id if user_id is not None else config.users.default_user_id
        # 假设 get_stats 是异步方法，加 await
        graph_stats = await graph.get_stats(user_id)
        stats.update({
