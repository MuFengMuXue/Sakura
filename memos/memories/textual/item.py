...
class TextualMemoryItem(BaseModel):
    ...
    def _coerce_metadata(cls, v: Any):
        ...
        if isinstance(v, dict):
            # 复制一份，避免修改原始数据
            data = v.copy()
            # 如果存在内部 metadata 嵌套，则提取
            if "metadata" in data and isinstance(data["metadata"], dict):
                nested = data["metadata"].copy()
                # 移除可能干扰的字段
                nested.pop("id", None)
                nested.pop("memory", None)
                data = nested
            else:
                # 否则移除顶层 id 和 memory
                data.pop("id", None)
            ...
            # ---- 兼容处理 internal_info ----
            if "internal_info" in data:
                val = data["internal_info"]
                if isinstance(val, str):
                    try:
                        data["internal_info"] = json.loads(val)
            ...
            # ---- 决定使用哪个 metadata 模型 ----
            if data.get("relativity") is not None:
                return SearchedTreeNodeTextualMemoryMetadata(**data)
            if any(k in data for k in ("sources", "memory_type", "embedding", "background", "usage")):
                return TreeNodeTextualMemoryMetadata(**data)
            return TextualMemoryMetadata(**data)
